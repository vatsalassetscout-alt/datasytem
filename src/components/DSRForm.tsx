...
export default function DSRForm({
  ...
  const handleSendNoteToAdmin = () => {
    if (adminMessageText.trim() && onSendAdminMessage) {
      onSendAdminMessage(adminMessageText.trim());
      setIsNoteSent(true);
      ...
      {isSuccess ? (
        ...
        <>
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {/* ✉️ Handover Note / Message to Admin Card */}
            <div className="bg-white p-6 rounded-3xl border border-gray-150 shadow-xs space-y-4">
              <div className="flex items-start sm:items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-700 font-extrabold shrink-0">
                  📬
                </div>
                <div>
                  <h3 className="font-extrabold text-gray-950 text-sm">Send Special Note / Handover Message to Admin</h3>
                  <p className="text-[10px] text-gray-500 font-medium">Convey blockages, questions, or updates directly to the admin. Sending this note will ping the Admin's notification bell immediately.</p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <textarea
                  ...
                  placeholder="e.g. Hi Admin, I finished the guest posting list today, but need clearance on the client blog spreadsheet for further backlinks. Please check."
                  onChange={(e) => setAdminMessageText(e.target.value)}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 focus:border-indigo-600 focus:bg-white rounded-xl text-xs text-gray-950 font-medium placeholder-gray-400 focus:outline-none transition leading-relaxed shadow-3xs"
                />
              </div>

              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-1">
                <span className="text-[10px] text-gray-405 font-bold font-mono uppercase">
                  {adminMessageText.trim() ? "✨ Ready to submit or dispatch instantly" : "✍️ Optional message area"}
                </span>

                <div className="flex gap-2 w-full sm:w-auto">
                  <button
                    type="button"
                    onClick={handleSendNoteToAdmin}
                    disabled={!adminMessageText.trim()}
                    className={`px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition select-none grow sm:grow-0 ${
                      adminMessageText.trim() 
                        ? 'bg-emerald-50 text-emerald-700 hover:bg-emerald-600 hover:text-white border border-emerald-200 cursor-pointer shadow-3xs' 
              ...
              {isNoteSent && (
                <div className="p-3 bg-emerald-50 border border-emerald-150 rounded-xl text-emerald-800 text-xs font-bold text-center animate-fade-in-up">
                  ✓ Note dispatched successfully to the Admin Alert System!
                </div>
              )}
            </div>
